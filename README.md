...
検索画面で並び順（おすすめ順／評価が高い順／口コミが多い順／駅から近い順）と
件数（10件／20件）を指定でき、バックエンドはその条件で並べた**上位N件だけ**を返す
（`SearchRequest.sort_by` / `SearchRequest.limit`、`api/places.py`）。
結果画面のプルダウンでも同じ並び替えを再取得なしでその場に適用できる（こちらは
一度取得した結果をフロント側で並び替えるだけ）。
